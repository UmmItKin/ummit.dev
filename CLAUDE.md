...
- **Humanize all English prose** — whenever you write or edit any user-facing English writing (posts, page copy, UI strings, commit messages, PR text), run it through the `/humanizer:humanizer` skill before you finish. This is not optional and applies to everything, not just CTF writeups. The recurring tells in this repo's old content are staged run-ups (`let's dive in`, `before we dive into`), empty closers (`Happy hacking!`, `unleashing its full potential`), sales words (`vibrant`, `seamless`, `elevate`, `empower`, `streamline`, `captivating`, `trusty companion`), `when it comes to` / `in the world of` filler, and forced `not X but Y` contrasts. Also no rule-of-three, no mechanical bold emphasis, vary sentence length, prefer plain `is/are/has`. Humanize prose only: leave code, frontmatter values, flags, and data untouched. Skip CJK content (Cantonese/Chinese posts and writeups) entirely. The em-dash in a writeup **title** is house style and stays.
